...
const SocialMediaWidget = () => {
  ...
  const socialLinks = [
    ...
    { 
      ...
      size: 'w-14 h-14', // Keep button size larger
    ...
    { 
      ...
      tooltip: 'Chat with an attendant and get super fast replies',
      delay: '300ms',
        ...
        {socialLinks.map((social, index) => (
          <div key={social.name} className={`transform transition-all duration-300 ${socialExpanded ? 'translate-y-0' : 'translate-y-4'}`}
            ...
            <div className="relative group flex items-center">
              ...
              <a href={social.href} className={`flex items-center justify-center ${social.name === 'Snapchat' ? social.size || 'w-12 h-12' : 'w-12 h-12'} ${social.bgClass} ${social.name === 'Snapchat' ? 'text-black' : 'text-white'} rounded-full shadow-lg hover:shadow-xl transition-all duration-300 hover:scale-110`}>
                <svg 
                  className={social.name === 'Snapchat' ? 'w-5 h-5' : 'w-6 h-6'} 
                  fill={social.name === 'Email' ? 'none' : (social.name === 'Snapchat' ? 'black' : 'currentColor')} 
                  stroke={social.name === 'Email' ? 'currentColor' : (social.name === 'Snapchat' ? 'black' : 'none')} 
                  strokeWidth={social.name === 'Email' ? '2' : (social.name === 'Snapchat' ? '1.2' : '0')} 
                  viewBox={social.name === 'Snapchat' ? '0 0 16 16' : '0 0 24 24'}
                >
                  {social.icon}
                </svg>
              </a>
