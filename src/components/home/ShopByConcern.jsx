...
const ShopByConcern = () => {
  ...
  // Calculate carousel width and card width based on screen size
  useEffect(() => {
    const updateDimensions = () => {
      if (carouselRef.current) {
        ...
        if (window.innerWidth < 640) {
          ...
          setCardWidth(280); // Small tablets
        } else if (window.innerWidth < 1024) {
          setCardWidth(300); // Medium tablets
        } else {
          setCardWidth(350); // Desktop
        }
      }
    };
  ...
  // Auto-scroll animation with improved smoothness
  useEffect(() => {
    ...
    const startAutoScroll = () => {
      ...
      const animate = (time) => {
        ...
        // Speed control - adjusted for responsive design
        const baseSpeed = 0.03;
        const speedFactor = window.innerWidth < 640 ? 0.7 : 1; // Slower on mobile
        const speed = baseSpeed * speedFactor;
        progress += deltaTime * speed;
        
        // Ensure smooth looping by resetting progress when reaching the end
        if (progress >= width) {
          progress = 0;
        }
  ...
  // Function to navigate to shop with concern filter
  const handleConcernClick = (concern, e) => {
    ...
    // Navigate to shop page with concern filter
    navigate('/shop', { 
      state: { 
  ...
  const getScrollAmount = () => {
    if (window.innerWidth < 640) {
      return cardWidth + 16; // Mobile card width + gap
    } else if (window.innerWidth < 768) {
      return cardWidth + 20; // Small tablet card width + gap
    } else {
      return cardWidth + 24; // Desktop card width + gap
    }
  };

  // Enter/leave viewport animations
  ...
  return (
    <section className="py-8 sm:py-12 md:py-16 bg-gray-50 overflow-hidden" ref={containerRef}>
      ...
      >
        ...
        {/* Main carousel container with centered navigation controls */}
        <div 
        ...
        >
          ...
          {/* Left navigation control - positioned absolutely */}
          <button 
          ...
          {/* Right navigation control - positioned absolutely */}
          <button 
        ...
        {/* Touch indicator (mobile and small tablet only) */}
