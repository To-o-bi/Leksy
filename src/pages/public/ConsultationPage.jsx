...
// Bulletproof consultation service that works with existing backend
const consultationService = {
  async initiateConsultation(consultationData) {
    ...
    // Build the params object with strict validation
    const params = {};
    
    try {
      // Required fields with strict validation
      params.name = cleanString(consultationData.name, 'Name');
      params.email = cleanString(consultationData.email, 'Email');
      params.phone = cleanString(consultationData.phone, 'Phone');
      params.age_range = cleanString(consultationData.age_range, 'Age range');
      params.gender = cleanString(consultationData.gender, 'Gender');
      params.skin_type = cleanString(consultationData.skin_type, 'Skin type');
      params.channel = cleanString(consultationData.channel, 'Channel');
      params.date = cleanString(consultationData.date, 'Date');
      params.time_range = cleanString(consultationData.time_range, 'Time range');
      
      // Handle skin_concerns specially
      let skinConcerns = consultationData.skin_concerns;
      if (isEmpty(skinConcerns)) {
        throw new Error('At least one skin concern is required');
      }
      ...
      const validConcerns = skinConcerns
        .filter(concern => !isEmpty(concern))
        .map(concern => concern.toString().trim());
      
      if (validConcerns.length === 0) {
        throw new Error('At least one valid skin concern is required');
      }
      
      params.skin_concerns = validConcerns.join(',');
      
      // Add success_redirect
      params.success_redirect = consultationData.success_redirect || `${window.location.origin}/consultation/success`;
      
      // Optional fields - only add if they have content
      if (!isEmpty(consultationData.current_skincare_products)) {
        params.current_skincare_products = consultationData.current_skincare_products.toString().trim();
      }
      
      if (!isEmpty(consultationData.additional_details)) {
        params.additional_details = consultationData.additional_details.toString().trim();
      }
    ...
    // Final validation of specific values
    const validChannels = ['video-channel', 'whatsapp'];
    if (!validChannels.includes(params.channel)) {
      throw new Error(`Invalid channel: "${params.channel}". Valid options: ${validChannels.join(', ')}`);
    }
    ...
    if (!validTimeRanges.includes(params.time_range)) {
      throw new Error(`Invalid time range: "${params.time_range}". Valid options: ${validTimeRanges.join(', ')}`);
    }
    ...
    // Log each parameter to ensure nothing is empty
    console.log('🔍 Parameter check:');
    Object.entries(params).forEach(([key, value]) => {
      console.log(`  ${key}: "${value}" (${typeof value}) - ${isEmpty(value) ? '❌ EMPTY' : '✅ OK'}`);
    });
    
    try {
      console.log('🚀 Making API request...');
      
      // Try direct POST with URL-encoded body (like contact form)
      const response = await fetch('https://leksycosmetics.com/api/consultation/initiate', {
        method: 'POST',
        headers: {
...
const ConsultationPage = () => {
  ...
  // Map display time slots to API time ranges
  const timeSlotMapping = {
    '9:00 AM': '2:00 PM - 3:00 PM',   // Map to available API slots
  ...
  // Watch selected skin concerns to limit to 3
  ...
  const onSubmit = async (data) => {
    ...
    try {
      // Prepare data for API with exact field mapping
      const consultationData = {
        ...
        time_range: timeSlotMapping[data.timeSlot],
        success_redirect: `${window.location.origin}/consultation/success`
      };

      console.log('🔄 Prepared consultation data:', consultationData);

      // Use the bulletproof consultation service
      const result = await consultationService.initiateConsultation(consultationData);
              ...
              {step === 3 && (
                <div className="space-y-6">
                  ...
                  <div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                      <label className="flex items-center justify-between p-4 border rounded-md cursor-pointer hover:bg-pink-50">
                        <div className="flex items-center">
                          <input
                            ...
                            className="h-4 w-4 text-pink-500 focus:ring-pink-500"
                          />
                          <span className="ml-2">Video Call (Zoom or Google Meet)</span>
                        </div>
                        <span className="text-pink-600 font-semibold">₦35,000</span>
                      </label>
                      <label className="flex items-center justify-between p-4 border rounded-md cursor-pointer hover:bg-pink-50">
                        <div className="flex items-center">
                          <input
                            ...
                            className="h-4 w-4 text-pink-500 focus:ring-pink-500"
                          />
                          <span className="ml-2">WhatsApp Consultation</span>
                        </div>
                        <span className="text-pink-600 font-semibold">₦15,000</span>
                      </label>
                    </div>
              ...
              <div className="mt-8 flex justify-between">
                ...
                {step < 3 ? (
                  ...
                  <button
                    type="submit"
                    disabled={isSubmitting}
                    className="ml-auto px-6 py-2 bg-pink-500 text-white rounded-md hover:bg-pink-600 disabled:opacity-50 disabled:cursor-not-allowed"
                  >
