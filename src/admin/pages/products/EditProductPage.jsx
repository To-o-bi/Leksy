...
const validateImageFile = (file) => {
  ...
  if (file.size > IMAGE_CONFIG.maxSize) {
    return { valid: false, error: `File is too large (max ${IMAGE_CONFIG.maxSize / 1024 / 1024}MB).` };
  }
  return { valid: true };
};
...
const FormField = React.memo(({ 
  ...
  return (
    <div className={className}>
      ...
      {type === 'select' ? (
        <select id={name} name={name} value={value} onChange={onChange} disabled={disabled} className={baseClasses} {...props}>
        ...
        <textarea id={name} name={name} value={value} onChange={onChange} disabled={disabled} rows={rows || 4} className={baseClasses} placeholder={placeholder} {...props} />
      ) : (
        <div className={currency ? 'relative' : ''}>
          ...
          <input type={type} id={name} name={name} value={value} onChange={onChange} disabled={disabled} className={`${baseClasses} ${currency ? 'pl-8' : ''}`} placeholder={placeholder} {...props} />
        </div>
      )}
...
const EditProductPage = () => {
  ...
  const validateForm = useCallback(() => {
    const newErrors = {};
    const validations = {
      name: { required: true, minLength: 2, message: 'Product name is required and must be at least 2 characters' },
      price: { required: true, type: 'number', min: 0.01, message: 'Please enter a valid price greater than 0' },
      description: { required: true, minLength: 10, message: 'Description is required and must be at least 10 characters' },
      quantity: { required: true, type: 'integer', min: 0, message: 'Please enter a valid quantity (0 or greater)' },
      category: { required: true, enum: PRODUCT_CATEGORIES, message: 'Please select a valid category' }
    };
    ...
    Object.entries(validations).forEach(([field, rules]) => {
      ...
      if (value) {
        if (rules.minLength && value.length < rules.minLength) newErrors[field] = rules.message;
        else if (rules.type === 'number' && (isNaN(value) || parseFloat(value) < (rules.min || 0))) newErrors[field] = rules.message;
        else if (rules.type === 'integer' && (isNaN(value) || parseInt(value, 10) < (rules.min || 0))) newErrors[field] = rules.message;
        else if (rules.enum && !rules.enum.includes(value)) newErrors[field] = rules.message;
      }
    });
    
    if (formData.slashed_price) {
      const slashedPrice = parseFloat(formData.slashed_price);
      if (isNaN(slashedPrice) || slashedPrice <= 0) newErrors.slashed_price = 'Please enter a valid original price';
      else if (formData.price && slashedPrice <= parseFloat(formData.price)) newErrors.slashed_price = 'Original price must be greater than current price';
    }
    ...
    <div>
      <div className="max-w-4xl mx-auto">
        <div className="bg-white rounded-lg shadow-md overflow-hidden">
          ...
          <div className="bg-white px-6 py-4 border-b border-gray-200">
            <div className="flex items-center">
              <button onClick={handleGoBack} className="flex items-center text-gray-600 hover:text-gray-800 mr-4 transition-colors" disabled={submitting}>
                <ChevronLeft size={20} />
                <span className="ml-1">Back</span>
              </button>
          ...
          <div className="px-6 py-6">
            {/* Status Messages */}
            {submitSuccess && <StatusMessage type="success" message="Product updated successfully!" submessage="Redirecting to products list..." />}
            {submitError && <StatusMessage type="error" message={submitError} />}
            
            {/* Form */}
            <form onSubmit={handleSubmit} className="space-y-6">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <FormField key="name" label="Product Name" name="name" required placeholder="Enter product name" value={formData.name} error={errors.name} onChange={handleChange} disabled={submitting} />
                ...
                <FormField key="price" label="Price" name="price" type="number" required currency placeholder="0.00" min="0" step="0.01" value={formData.price} error={errors.price} onChange={handleChange} disabled={submitting} />
                <FormField key="slashed_price" label="Original Price (Optional)" name="slashed_price" type="number" currency placeholder="0.00" min="0" step="0.01" value={formData.slashed_price} error={errors.slashed_price} onChange={handleChange} disabled={submitting} />
                <FormField key="quantity" label="Quantity" name="quantity" type="number" required placeholder="Enter available quantity" min="0" className="md:col-span-2" value={formData.quantity} error={errors.quantity} onChange={handleChange} disabled={submitting} />
                <FormField key="description" label="Description" name="description" type="textarea" required placeholder="Enter product description" className="md:col-span-2" value={formData.description} error={errors.description} onChange={handleChange} disabled={submitting} />
                
                {/* Images Section */}
                {existingImages.length > 0 && (
                  <div className="md:col-span-2">
                    <label className="block text-sm font-medium text-gray-700 mb-3">Current Images ({existingImages.length})</label>
                    <ImageGrid images={existingImages} type="existing" onRemove={handleRemoveImage} submitting={submitting} />
                  </div>
                )}
                ...
                <div className="md:col-span-2">
                  ...
                  <div className={`border-2 border-dashed rounded-lg p-6 text-center transition-colors ${errors.images ? 'border-red-300 bg-red-50' : 'border-gray-300 hover:border-gray-400'}`}>
                    <Upload className="mx-auto h-12 w-12 text-gray-400 mb-4" />
                    <div className="space-y-2">
                      <label htmlFor="images" className="cursor-pointer text-blue-600 hover:text-blue-500 font-medium">
                        Choose files
                        <input id="images" name="images" type="file" ref={fileInputRef} className="hidden" multiple accept="image/*" onChange={handleImageChange} disabled={submitting} />
                      </label>
                      <p className="text-gray-500">or drag and drop</p>
                      <p className="text-sm text-gray-400">PNG, JPG, WebP up to 2MB each (max {IMAGE_CONFIG.maxCount} total)</p>
                    </div>
                  </div>
                  ...
                  {newImagePreviews.length > 0 && (
                    <div className="mt-4">
                      <p className="text-sm font-medium text-gray-700 mb-3">New Images ({newImages.length}):</p>
                      <ImageGrid images={newImagePreviews} type="new" onRemove={handleRemoveImage} submitting={submitting} />
                    </div>
                  )}
                </div>
              </div>
              ...
              <div className="flex justify-end space-x-3 pt-6 border-t border-gray-200">
                <button type="button" onClick={handleGoBack} className="px-6 py-2 border border-gray-300 rounded-md text-sm font-medium text-gray-700 bg-white hover:bg-gray-50 transition-colors" disabled={submitting}>
                  Cancel
                </button>
                <button type="submit" disabled={submitting || !hasChanges()} className={`px-6 py-2 border border-transparent rounded-md text-sm font-medium text-white transition-colors ${submitting || !hasChanges() ? 'bg-gray-400 cursor-not-allowed' : 'bg-blue-600 hover:bg-blue-700'}`}>
                  {submitting ? (
                    <div className="flex items-center">
